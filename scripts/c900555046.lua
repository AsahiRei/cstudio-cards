...
function s.initial_effect(c)
    --dal procedure
    DateALive.Lv3Procedure(c,id,CODE_IRREGULAR_AI)
    --pendulum summon
...
s.listed_names={CODE_IRREGULAR_AI,CODE_SPACEQUAKE}
function s.thfilter(c)
	return c:IsSetCard(SET_DAL) and c:IsType(TYPE_SPELL) and c:IsAbleToHand()
end
...
function s.pzfilter(c)
	return c:IsCode(CODE_IRREGULAR_AI) and not c:IsForbidden()
end
...
function s.spcfilter(c,tp)
	return c:IsSetCard(SET_DAL) and c:IsType(TYPE_SPELL) and not c:IsPublic()
end
